...
type primitive = void | boolean | number | string | primitiveObj | primitiveArray
interface primitiveObj { [name: string]: primitive }
interface primitiveArray extends Array<primitive> {}

/** Pack raw data to a buffer. */
export const pack = (data: primitive) => encode(data, {codec}) as buffer

/** Unpack a buffer to raw data. */
export const unpack = (buffer: buffer) => decode(buffer, {codec}) as primitive
